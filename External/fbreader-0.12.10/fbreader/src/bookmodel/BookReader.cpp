...
void BookReader::setFilePrefix(const std::string &prefix)
{
    myModel.myBook->setFilePrefix(prefix);
};
...
void BookReader::addContentsData(const std::string &data) {
	if (!data.empty() && !myTOCStack.empty()) {
		shared_ptr<Content> content = new Content(data, 0, 0, "");
		myModel.myBook->addContent(content);
...
void BookReader::flushTextBufferToParagraph() {
    //Just save the string, but may not a whole chapter.
	myCurrentTextModel->addText(myBuffer);
	myModel.myBook->addParagraph(myBuffer);
	myBuffer.clear();
}
...
void BookReader::addImage(std::string imageFile)
{
    myModel.myBook->addImageFile(imageFile);
}

void BookReader::addHRef(std::string hRef)
{
    myModel.myBook->addHRef(hRef);
}
...
void BookReader::addContentsToBook(shared_ptr<Content> content) {
	myModel.myBook->addContent(content);
}

//void BookReader::addBookInnerFileStream( const std::string filePath, shared_ptr<ZLInputStream> inputStream) {
//	myModel.myBook->addBookInnerFileStream(filePath, inputStream);
//}

void BookReader::addFirstChapter(const std::string &strHref)
{
    myModel.myBook->addFirstChapter(strHref);
}
