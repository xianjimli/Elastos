...
void testAvailable()
{
	...
	CSequenceInputStream::New(sbis1, sbis2, (IInputStream **)&is);
    is->Available(&number);
	printf("number=%d\n",number);//7 问题：it should be 11,currently it only includes the first inputstream,not consisting of the second stream .
	is->Close();
    is->Available(&number);	
	printf("number=%d\n",number);//0
}
...
void testReadBufferEx()
{
	...
	{
	    printf("%c ",bytes[i]);
	}//a b c d e f g //问题：应为：a b c d e f g j h h
     //number=7  //问题：应该为10才对
	printf("\n");
	printf("number=%d\n",number);
}
