...
void testRead()
{
    ...
    printf("value=%c\n",value);	//c
    fis->Read(&value);
    printf("value=%c\n",value);//d  //问题：after Mark(1),it limit 1 character can be read,but it still read the next character "d";
    fis->Read(&value);
    printf("value=%d\n",value);//-1
	fis->Reset();//问题：after Reset().did not reposition to the last Marked position
    fis->Read(&value);
    printf("value=%c\n",value);//a
}
void testReadBuffer()
