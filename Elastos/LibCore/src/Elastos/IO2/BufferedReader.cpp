    ...
    /*[out]*/ Int32* number)
{
    ...
    if (mMark == 0 && mMarkLimit > length) {
        ...
        BufferOf<Byte>* newbuf = BufferOf<Byte>::Alloc(newLength);
    ...
    Int32 count;
    FAIL_RETURN(mIn->ReadBufferEx(mPos, mBuf->GetCapacity() - mPos, mBuf, &count));
    if (count != -1) {
        mEnd += count;
    }
    *number = count;
    ...
    /*[out]*/ Int32* number)
{
    ...
    FAIL_RETURN(CheckNotClosed());
    if (offset < 0 || offset > buffer->GetCapacity() - length || length < 0) {
        ...
        /*[out]*/ String* contents)
{
    ...
    for (Int32 charPos = mPos; charPos < mEnd; charPos++) {
        ...
        if (ch == '\n') {
            Int32 length = mBuf->GetCapacity();
            StringBuf* strbuf = StringBuf::Alloc((char*)mBuf->GetPayload(), length);
            StringBuf_<10000> substrbuf;
            String res = strbuf->Substring(mPos, charPos - mPos, substrbuf);
            *contents = String::Duplicate(res);
            ...
            if (((mPos < mEnd) || number2 != -1)
                    && String((char*)mBuf->GetPayload()).GetChar(mPos) == '\n') {
                mPos++;
            }
            *contents = String::Duplicate(res);
            StringBuf::Free(strbuf);
    ...
    StringBuf_<80> result;
    /* Typical Line Length */

    // result.append(buf, pos, end - pos);
    Int32 length = mBuf->GetCapacity();
    StringBuf* strbuf = StringBuf::Alloc((char*)mBuf->GetPayload(), length);
    String res = strbuf->Substring(mPos, mEnd - mPos, result);
    StringBuf_<10000> result2;
    String res2 = strbuf->Substring(0, length, result2);
    while (TRUE) {
        mPos = mEnd;
        ...
        for (Int32 charPos = mPos; charPos < mEnd; charPos++) {
            Char32 c = String((char*)mBuf->GetPayload()).GetChar(charPos);
            if (eol == '\0') {
                if ((c == '\n' || c == '\r')) {
                    eol = c;
                }
                ...
                String res3;
                StringBuf_<80> substrbuf3;
                if (charPos > mPos) {
                    result.Append(res2, mPos, charPos - mPos - 1);
                    res3 = result.Substring(0, result.GetLength(),substrbuf3);
                }
                mPos = charPos + 1;
                // return result.toString();
                *contents = String::Duplicate(res3);
                StringBuf::Free(strbuf);
                return NOERROR;
            } else {
                String res4;
                StringBuf_<80> substrbuf4;
                if (charPos > mPos) {
                    result.Append(res2, mPos, charPos - mPos - 1);
                    res4 = result.Substring(0, result.GetLength(),substrbuf4);
                }
                mPos = charPos;
                // return result.toString();
                *contents = String::Duplicate(res4);
                StringBuf::Free(strbuf);
                return NOERROR;
            }
        }
