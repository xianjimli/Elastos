...
struct DirEntry
{
    ...
    {
        name = AutoString::Duplicate(filename);
        next = NULL;
    }
    ...
    /* [in] */ String dirtyPath)
{
    ...
    AutoString cleanPath;
    FixSlashes(dirtyPath, &cleanPath);
    Boolean isAbsolute = cleanPath.GetLength() > 0 && cleanPath.GetChar(0) == separatorChar;
    if (isAbsolute) {
        mPath = AutoString::Duplicate(cleanPath);
        mAbsolutePath = AutoString::Duplicate(cleanPath);
    }
    else {
//        String userDir = AccessController.doPrivileged(new PriviAction<String>("user.dir"));
        String userDir("");
        if (cleanPath.IsEmpty()) {
            mAbsolutePath = AutoString::Duplicate(userDir);
        }
        else {
            String temp;
        ...
        mPath = AutoString::Duplicate(mAbsolutePath.Substring(
    ...
    /* [out] */ String* destPath)
{
    ...
    if (Character::GetByteCount(separatorChar)
        ...
        if (newLength != size) {
            (*newPath)[newLength] = '\0';
            *destPath = String::Duplicate(*newPath);
        }
        else {
            *destPath = String::Duplicate(origPath);
        }
        ...
        for (Int32 i = 0; i < size; ++i) {
            ...
            if (ch == '/') {
                if (!lastWasSlash) {
                    Int32 number;
                    BufferOf<Byte> buf((Byte*)newPath->GetPayload(),
                            newPath->GetCapacity());
    ...
    /* [out] */ String* dest)
{
    ...
    Boolean haveSlash = (prefixLength > 0 && prefix.GetChar(prefixLength - 1) == separatorChar);
    if (!haveSlash) {
        haveSlash = (suffix.GetLength() > 0 && suffix.GetChar(0) == separatorChar);
    }
    StringBuffer strbuf;
    if (haveSlash) {
        strbuf += prefix;
...
{
    assert(path != NULL);
    *path = String::Duplicate(mAbsolutePath);
    return NOERROR;
}
    ...
    /* [out] */ String* path)
{
    ...
    if (separatorChar != '/') {
        if (result.GetChar(0) == '\\') {
            rootLoc = (result.GetLength() > 1 && result.GetChar(1) == '\\') ? 1 : 0;
        }
        else {
    ...
    BufferOf<Byte>* newResult = BufferOf<Byte>::Alloc(result.GetCharCount() * 4 + 1);
    ...
    for (Int32 i = 0; i <= N; ++i) {
        if (i < rootLoc) {
            ...
            if (i == N || result.GetChar(i) == separatorChar) {
                if (i == N && foundDots == 0) {
                    break;
                }
                if (foundDots == 1) {
    ...
    *path = String::Duplicate((char*)newResult->GetPayload());
    delete sepLocations;
    BufferOf<Byte>::Free(newResult);
    return NOERROR;
}
    ...
    /* [out] */ String* destpath)
{
    ...
    Int32 last = 1;
    Boolean done;
    for (Int32 i = 1; i <= path.GetCharCount(); i++) {
        AutoString linkPath = static_cast<AutoString>(AutoString::Duplicate(path));
        AutoString bytes;
        if (i == path.GetCharCount() || path.GetChar(i) == separatorChar) {
            done = i >= path.GetCharCount() - 1;
            // if there is only one segment, do nothing
            if (done && linkPath.GetCharCount() == 1) {
                *destpath = String::Duplicate(path);
                return NOERROR;
            }
            Boolean inPlace = FALSE;
            if (linkPath.Equals(path)) {
                bytes = AutoString::Duplicate(path);
                // if there are no symbolic links, truncate the path instead of copying
                if (!done) {
                    inPlace = TRUE;
                    Int32 offset;
                    Character::GetOffsetByChars(path, 0, i, &offset);
                    StringBuf* tmp = StringBuf::Alloc(offset);
                    //todo: memory leak
                    path = path.Substring(0, i, *tmp);
                }
            }
            else {
                Int32 nextSize = i - last + 1;
                Int32 linkSize = linkPath.GetCharCount();
                if (linkPath.GetChar(linkSize - 1) == separatorChar) {
                    linkSize--;
                }
                ...
                StringBuf* tmp1 = StringBuf::Alloc(offset1 + offset3 - offset2);
                StringBuf* tmp2 = StringBuf::Alloc(offset3 - offset2);
                linkPath.Substring(0, offset1, *tmp1);
                //todo: memory leak
                (*tmp1) << path.Substring(offset2, offset3, *tmp2);
                StringBuf::Free(tmp2);
                //todo: memory leak
                bytes = linkPath;
            ...
            if (inPlace) {
                // path[i] = '/';
                if (i + 1 < path.GetCharCount()) {
                    Int32 offset1, offset2;
                    Character::GetOffsetByChars(path, 0, i, &offset1);
                    Character::GetOffsetByChars(path, 0, i + 1, &offset2);
                    StringBuf* tmp1 = StringBuf::Alloc(offset1 + 1 + path.GetLength() - offset2);
                    path.Substring(0, i, *tmp1);
                    (*tmp1) << '/' << path.Substring(offset2);
                    //todo: memory leak
                    path = tmp1->GetPayload();
                }
                else {
                    Int32 offset;
                    Character::GetOffsetByChars(path, 0, i, &offset);
                    StringBuf* tmp1 = StringBuf::Alloc(offset + 1);
                    path.Substring(0, i, *tmp1);
                    (*tmp1) << '/';
                    //todo: memory leak
                    path = tmp1->GetPayload();
    ...
    /* [out] */ String* destlink)
{
    ...
    do {
        AutoString fragment, target;
        Int32 offset;
        Character::GetOffsetByChars(path, 0, length, &offset);
        //todo: memory leak
        StringBuf* tmp = StringBuf::Alloc(offset + 1);
        fragment = path.Substring(0, offset, *tmp);
        Readlink(fragment, &target);
        if (target.Equals(fragment)) {
            break;
        }
        if (target.GetChar(0) == separatorChar) {
            // The link target was an absolute path, so we may need to start again.
            restart = resolveAbsolute;
            Int32 offset;
            Character::GetOffsetByChars(path, 0, length, &offset);
            StringBuf* tmp = StringBuf::Alloc(target.GetLength() + path.GetLength() - offset);
            (*tmp) << target << path.Substring(offset);
            //todo: memory leak
            path = tmp->GetPayload();
        }
        else {
            Int32 offset = path.LastIndexOfChar(separatorChar) + 1;
            StringBuf* tmp = StringBuf::Alloc(offset + target.GetLength());
            path.Substring(0, offset, *tmp);
            (*tmp) << target;
            //todo: memory leak
            path = tmp->GetPayload();
        }
    ...
    *destlink = String::Duplicate(path);
    return NOERROR;
}
    ...
    /* [out] */ String* destLink)
{
    ...
    while (TRUE) {
        ...
        if (len == -1) {
            // An error occurred.
            *destLink = String::Duplicate(filePath);
        ...
        if (static_cast<size_t>(len) < static_cast<size_t>(buf->GetLength() - 1)) {
            ...
            *destLink = String::Duplicate(buf->GetPayload());
            ArrayOf<Char8>::Free(buf);
            return;
        }
    ...
    /* [out] */ String* name)
{
    ...
    if (separatorIndex < 0) {
        *name = String::Duplicate(mPath);
    }
    else {
        //todo: memory leak
        StringBuf* tmp = StringBuf::Alloc(mPath.GetLength() - (separatorIndex + 1));
        *name = mPath.Substring(separatorIndex + 1, mPath.GetLength(), *tmp);
    }
    return NOERROR;
}
    ...
    /* [out] */ String* parent)
{
    ...
    Int32 index = mPath.LastIndexOfChar(separatorChar);
    if (index == -1 && firstInPath > 0) {
        index = 2;
    }
    if (index == -1 || mPath.GetChar(charCount - 1) == separatorChar) {
        *parent = String(NULL);
        return NOERROR;
    }
    if (mPath.IndexOfChar(separatorChar) == index
            && mPath.GetChar(firstInPath) == separatorChar) {
        Int32 offset;
        Character::GetOffsetByChars(mPath, 0, index + 1, &offset);
        //todo: memory leak
        StringBuf* tmp = StringBuf::Alloc(offset);
        mPath.Substring(0, offset, *tmp);
        *parent = tmp->GetPayload();
        return NOERROR;
    }
    Int32 offset;
    ...
    /* [out] */ Boolean* isAbsolute)
{
    ...
    *isAbsolute = mPath.GetCharCount() > 0 && mPath.GetChar(0) == separatorChar;
    return NOERROR;
}
...
//    }
    ...
    *files = ArrayOf<String>::Alloc(_files.size());
    for (Int32 i = 0; _files.size() != 0; _files.pop_front(), ++i) {
        String fname = String::Duplicate(_files.front());
        (**files)[i] = fname;
    }
    return NOERROR;
}
    ...
    /* [out] */ String* absoluteName)
{
    ...
    StringBuf* strBuf = StringBuf::Alloc(name.GetLength() + 1);
    if (isDir && name.GetChar(name.GetCharCount() - 1) != separatorChar) {
...
//        name = name.replace(separatorChar, '/');
    ...
    *absoluteName = String::Duplicate(name);
    return NOERROR;
}
