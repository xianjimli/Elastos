    ...
    /* [in] */ Int32 i)
{
    ...
    if (mCount + i <= mBuf->GetCapacity()){
        return NOERROR;
    }
    BufferOf<Byte>* newbuf = BufferOf<Byte>::Alloc((mCount + i) * 2);
...
ECode ByteArrayOutputStream::ToByteArray(
    /* [out, callee] */ BufferOf<Byte>** byteArray)
{
    assert(byteArray != NULL);
    BufferOf<Byte>* newArray = BufferOf<Byte>::Alloc(mCount);
