...
//      throw new IndexOutOfBoundsException();
    ...
    if (mPos < mBuf->GetCapacity()) {
        copyLength = (mBuf->GetCapacity() - mPos >= count) ? count : mBuf->GetCapacity()
                - mPos;
...
//      throw new IOException("Reader is closed");
    ...
    *ready = (mBuf->GetCapacity() - mPos > 0 || *ready);
...
{
    FAIL_RETURN(UnReadBufferEx(0, buffer.GetCapacity(), buffer));
    return NOERROR;
}
...
//      throw new IOException("Pushback buffer full");
    ...
    if (offset > buffer.GetCapacity() - length || offset < 0) {
...
//      throw new IllegalArgumentException();
    ...
    Int64 inSkipped;
    Int32 availableFromBuffer = mBuf->GetCapacity() - mPos;
...
{
    if(mBuf->IsNull()) return E_IO_EXCEPTION;
    return NOERROR;
}