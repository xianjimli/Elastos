...
//      throw new IllegalArgumentException("size <= 0");
    ...
    if(in != NULL) mBuf = BufferOf<Byte>::Alloc(size);
    mPos = size;
...
//      throw new IOException();
    ...
    *number = mBuf->GetCapacity() - mPos + al;
...
//      throw new ArrayIndexOutOfBoundsException("Length out of bounds: " + length);
    ...
    if (mPos < mBuf->GetCapacity()) {
        copyLength = (mBuf->GetCapacity() - mPos >= length) ? length : mBuf->GetCapacity()
                - mPos;
...
//      throw streamClosed();
    ...
    Int32 numSkipped = 0;
    if (mPos < mBuf->GetCapacity()) {
        numSkipped += (count < mBuf->GetCapacity() - mPos) ? count : mBuf->GetCapacity() - mPos;
        mPos += numSkipped;
    }
    if (numSkipped < count) {
        Int64 num;
...
{
    return UnReadBufferEx(0, buffer.GetCapacity(), buffer);
}
