    ...
    /* [out] */ Int32* number)
{
    assert(buffer != NULL);
    assert(number != NULL);
    return mIn->ReadBufferEx(0, buffer->GetCapacity(), buffer, number);
}
...
ECode DataInputStream::ReadFully(
    /* [out] */ BufferOf<byte> * buffer)
{
    return ReadFullyEx(0, buffer->GetCapacity(), buffer);
}
    ...
    while (TRUE) {
        ...
        switch (nextByte) {
            case -1:
                ...
                *str = String::Duplicate(line->GetPayload());
                StringBuf::Free(line);
                return NOERROR;
            case (Byte)'\r':
                if (foundTerminator) {
