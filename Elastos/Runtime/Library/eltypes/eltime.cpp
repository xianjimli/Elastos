...
Boolean __cdecl _DateTime_IsValid(PDateTime pDateTime)
{
    ...
    if ((pDateTime->year % 100 != 0 && pDateTime->year % 4 == 0) || pDateTime->year % 400 == 0) {
    ...
    if(pDateTime->dayofweek != ((pDateTime->day + 2 * month +
        3 * (month + 1) / 5 + year + year / 4 - year / 100 + year / 400 + 1) % 7)) {
        return FALSE;
    }
        ...
        /* [out] */ PDateTime pDateTime)
{
    ...
    UInt32 month = 1;
    UInt8 *  DaysInMonth = IS_LEAP_YEAR(year + 300) ? s_DaysInMonth1 : s_DaysInMonth0;
    while (1) {
        ...
        /* [out] */ PInt64 pTimestamp)
{
    ...
    Int64 seconds = (((
        (UInt32)(year/4 - year/100 + year/400 + 367*month/12 + pDateTime->day) +
        year*365 - 719499
...
static const char  day_name[][4] = {"Sun", "Mon",
    "Tue", "Wed", "Thu", "Fri", "Sat"};
...
PCarQuintet __cdecl _StringBuf_Append_DateTime(PCarQuintet pCq, const char *fmt, PDateTime pDateTime)
{
    assert(pCq && fmt);
    ...
    while (*fmt) {
        switch (getFmtA(&fmt)) {
            ...
            case DateTimeFlag_Week       :
                _StringBuf_Append_String(pCq, day_name[pDateTime->dayofweek], 0, 3);
                break;
...
PDateTime __cdecl _DateTime_Parse_String(PDateTime pDateTime, const char* time, const char* fmt)
{
