...
CAR_INLINE PCarQuintet __cdecl _CarQuintet_Alloc(Int32 size)
{
...
PCarQuintet __cdecl _CarQuintet_Clone(const PCarQuintet pCq)
{
    ...
    if (pNewCq) {
        CarQuintetFlags flags = pCq->m_flags | CarQuintetFlag_HeapAlloced;
...
#ifndef _RELEASE
    if (pCq) memset(pCq->m_pBuf, 0xCC, size);
#endif
    return pCq;
}
