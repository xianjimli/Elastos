...
{
    ECode ec;
    InterfacePack *pItfPack;

    if (pObj == NULL || ppBuf == NULL || pSize == NULL) {
        return E_INVALID_ARGUMENT;
    }

    pItfPack = (InterfacePack*)calloc(sizeof(InterfacePack), 1);
    if (pItfPack == NULL) {
        ec = E_OUT_OF_MEMORY;
        goto ErrorExit;
    }

    ec = StdMarshalInterface(pObj, pItfPack);
    if (FAILED(ec)) goto ErrorExit;

    *pSize = sizeof(InterfacePack);
    *ppBuf = pItfPack;

    return NOERROR;

ErrorExit:
    if (pItfPack) free(pItfPack);
    *ppBuf = NULL;
    *pSize = 0;
    ...
    /* [out] */ Int32 *pSize)
{
    ...
    if (pBuf == NULL || ppObj == NULL || pSize == NULL) {
        return E_INVALID_ARGUMENT;
    }

    ec = StdUnmarshalInterface(flag, (InterfacePack*)pBuf, ppObj);
    if (FAILED(ec)) {
        *ppObj = NULL;
        *pSize = 0;
