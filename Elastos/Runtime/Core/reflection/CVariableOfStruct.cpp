    ...
    /* [out] */ IStructSetter ** ppSetter)
{
    ...
    IStructInfo *pStructInfo = NULL;
    ec = m_pStructInfo->GetFieldInfo(name, &pFieldInfo);
    if (FAILED(ec)) return ec;
    ...
    ec = pStructInfo->CreateVariableBox(
        m_pVarBuf + m_pStructFieldDesc[index].pos, &pVariable);
    pStructInfo->Release();
    if (FAILED(ec)) return ec;
    ...
    /* [out] */ IInterface ** ppSGetter)
{
    ...
    if (!pSGetter) {
        ...
        ec = pTypeInfo->GetElementTypeInfo(&pElementTypeInfo);
        pTypeInfo->Release();
        if (FAILED(ec)) {
            g_objInfoList.UnlockHashTable(EntryType_Struct);
            return ec;
