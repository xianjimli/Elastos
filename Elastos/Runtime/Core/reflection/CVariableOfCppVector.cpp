    ...
    /* [out] */ IStructSetter ** ppSetter)
{
    ...
    IVariableOfStruct *pVariable;
    ECode ec = pStructInfo->CreateVariableBox(
        (PByte)m_pVarPtr + m_iElementSize * index, &pVariable);
    if (FAILED(ec)) {
        return ec;
    }

    ec = pVariable->GetSetter(ppSetter);
    pVariable->Release();
    ...
    /* [out] */ Int32 *pRank)
{
    ...
    IDataTypeInfo *pPElementTypeInfo = m_pElementTypeInfo;
    if (type == CarDataType_CppVector) pPElementTypeInfo->AddRef();

    while (type == CarDataType_CppVector) {
        rank++;
        ec = ((ICppVectorInfo *)pPElementTypeInfo)->GetElementTypeInfo(
            &pElementTypeInfo);
        pPElementTypeInfo->Release();
    ...
    if (pElementTypeInfo) pElementTypeInfo->Release();
    ...
    /* [out] */ IStructGetter ** ppGetter)
{
    ...
    ec = pVariable->GetGetter(ppGetter);
    pVariable->Release();
