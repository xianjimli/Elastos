    ...
    /* [in] */ Boolean bCheckLocalPtr)
{
    ...
    *ppDataTypeInfo = NULL;
    ...
    //CppVectorInfo
        ...
        ec = AcquireDataTypeInfo(pCClsModule, pType, &pElemInfo,
            bCheckLocalPtr);
        if (FAILED(ec)) return ec;
    ...
    /* [out] */ ICppVectorInfo **ppCppVectorInfo)
{
    ...
    Int32 len = 0;
    IDataTypeInfo *pElementInfo = NULL;
    ...
    for (; pNode; pNode = pNode->pNext) {
        ...
        if (len == length) {
            ec = pCppVectorInfo->GetElementTypeInfo(&pElementInfo);
            if (FAILED(ec)) {
                UnlockHashTable(EntryType_DataType);
                return ec;
            }
            if (pElementInfo == pElementTypeInfo) {
                pElementInfo->Release();
    ...
    if (dataType != CarDataType_Struct) {
        ...
        *ppCarArrayInfo = m_pCarArrayInfos[dataType];
        ...
        for (; pNode; pNode = pNode->pNext) {
            ...
            ec = pCarArrayInfo->GetElementTypeInfo(&pElementInfo);
            if (FAILED(ec)) {
                UnlockHashTable(EntryType_DataType);
                return ec;
            }
