    ...
    /* [out] */ Boolean * pbHasGeneric)
{
    ...
    IClassInfo *pParentClassInfo = NULL;
    IClassInfo *pThisClassInfo = this;
    pThisClassInfo->AddRef();
    *pbHasGeneric = FALSE;
    ...
    while (TRUE) {
        ...
        if (bHasBaseClass) {
            ec = pThisClassInfo->GetBaseClassInfo(&pParentClassInfo);
            if (FAILED(ec)) goto Exit;
...
Exit:
    if (pThisClassInfo) pThisClassInfo->Release();
    if (pParentClassInfo) pParentClassInfo->Release();
    return ec;
}
...
ECode CClassInfo::AcquireConstructorList()
{
    ...
    if (!m_pCtorClassInfo) {
        ...
        ec = m_pCClsModule->GetModuleInfo(&pModuleInfo);
        if (FAILED(ec)) {
            g_objInfoList.UnlockHashTable(EntryType_Class);
            return ec;
        }
    ...
    /* [out] */ IConstructorInfo **ppConstructorInfo)
{
    ...
    IConstructorInfo *pConstructorInfo = NULL;
