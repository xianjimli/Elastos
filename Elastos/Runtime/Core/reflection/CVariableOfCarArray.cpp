    ...
    /* [in] */ Boolean bAlloc)
{
    ...
    m_pCarArrayInfo->GetElementTypeInfo(&pElementTypeInfo);
    ...
    /* [out] */ IStructSetter ** ppSetter)
{
    ...
    IDataTypeInfo  *pElementTypeInfo = NULL;
    ECode ec = m_pCarArrayInfo->GetElementTypeInfo(&pElementTypeInfo);
    if (FAILED(ec)) {
        return ec;
    }

    CStructRefInfo *pStructInfo = (CStructRefInfo *)pElementTypeInfo;

    IVariableOfStruct *pVariable;
    ec = pStructInfo->CreateVariableBox(
        (PByte)m_pCq->m_pBuf + m_iElementSize * index, &pVariable);
    pElementTypeInfo->Release();
    if (FAILED(ec)) {
        return ec;
    }
