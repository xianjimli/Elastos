    ...
    /* [out] */ PInterface *ppObject)
{
    ...
    char path[260];
#ifdef _linux
    sprintf(path, "/data/data/com.elastos.runtime/elastos/%s", pszUunm);
#else
    strcpy(path, pszUunm);
#endif
...
ELAPI _CProcess_FreeUnusedModule(String szDllName)
{
    ...
    while (pLocalModule != (LocalModule *)&g_LocModList) {
        ...
        else if (szDllName.Compare(
                    pLocalModule->m_pszUunm+nLen1-nLen2, StringCase_Insensitive)) {
            pLocalModule = pnext;
            continue;
        }
        else if (nLen1 != nLen2 &&
