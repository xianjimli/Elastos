...
const Int32 CMenuInflater::MenuState::mDefaultGroupId;
const Int32 CMenuInflater::MenuState::mDefaultItemId;
...
{
    AutoPtr<ITypedArray> a = NULL;
    mHost->mContext->ObtainStyledAttributesEx2(attrs,
        ArrayOf<Int32>(R_Styleable_MenuGroup, sizeof(R_Styleable_MenuGroup) / sizeof(Int32)),
        (ITypedArray**) &a);

    a->GetResourceId(1, mDefaultGroupId, &mGroupId);
    a->GetInt32(3, mDefaultItemCategory, &mGroupCategory);
    a->GetInt32(4, mDefaultItemOrder, &mGroupOrder);
    a->GetInt32(5, mDefaultItemCheckable, &mGroupCheckable);

    a->GetBoolean(2, mDefaultItemVisible, &mGroupVisible);
    a->GetBoolean(0, mDefaultItemEnabled, &mGroupEnabled);
	...
	/* [in] */ IAttributeSet* attrs)
{
    AutoPtr<ITypedArray> a = NULL;
    mHost->mContext->ObtainStyledAttributesEx2(attrs,
        ArrayOf<Int32>(R_Styleable_MenuItem, sizeof(R_Styleable_MenuItem) / sizeof(Int32)),
        (ITypedArray**) &a);
...
Char8 CMenuInflater::MenuState::GetShortcut(
    /* [in] */ String shortcutString)
{
    if (shortcutString.IsNull()) {
        return 0;
    } else {
        return shortcutString[0];
    }
}
...
{
    mItemAdded = TRUE;
    AutoPtr<ISubMenu> subMenu = NULL;
    AutoPtr<ICharSequence> csq = NULL;
	...
	/* [in] */ IMenuItem* item)
{
    ...
    if (mItemCheckable >= 2) {
        ((IMenuItemImpl*)item)->SetExclusiveCheckable(TRUE);
    }
}
    ...
    /* [in] */ IMenu* menu)
{
    ...
    //try {
	AutoPtr<IResources> resources = NULL;
    ...
    /* [in] */ IMenu* menu)
{
    ...
    do {
        if (eventType == IXmlPullParser_START_TAG) {
            ...
            //if (tagName == XML_MENU) {
            if (!XML_MENU.Compare((const char*) tagName)) {
                // Go to next tag
    ...
    while (!reachedEndOfMenu) {
        switch (eventType) {
            case IXmlPullParser_START_TAG:
                ...
                parser->GetName(&tagName);
                if (!XML_GROUP.Compare(CString((const char*) tagName))) {
                    menuState->ReadGroup(attrs);
                }
                else if (!XML_ITEM.Compare(CString((const char*) tagName))) {
                    menuState->ReadItem(attrs);
                }
                else if (!XML_MENU.Compare(CString((const char*) tagName))) {
            ...
            case IXmlPullParser_END_TAG:
                ...
                else if (!XML_GROUP.Compare((const char*) tagName)) {
                    menuState->ResetGroup();
                }
                else if (!XML_ITEM.Compare((const char*) tagName)) {
                ...
                else if (!XML_MENU.Compare((const char*) tagName)) {
